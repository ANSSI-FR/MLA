...
/// Fixed salt for Argon2 password derivation.
/// In password mode, sender and receiver derive the same key from the same password.
/// The salt is fixed and public -- security relies entirely on the password.
...
fn encrypt_with_password_impl(
    ...
    let (priv_key, pub_key) = derive_keypair_from_password(password).map_err(JsValue::from)?;
...
///
/// The password is used to derive an encryption keypair via Argon2id.
