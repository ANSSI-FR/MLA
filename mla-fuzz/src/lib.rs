//! Common fuzzing logic for MLA archives.
//!
//! This crate provides the shared fuzzing infrastructure used by both
//! AFL-based fuzzing (`mla-fuzz-afl`) and OSS-Fuzz/libFuzzer-based fuzzing (`mla-oss-fuzz`).
...
/// Test input structure for fuzzing
#[derive(Debug)]
pub struct TestInput {
    pub config: FuzzMode,
    pub filenames: Vec<String>,
    pub parts: Vec<Vec<u8>>,
    pub byteflip: Vec<u32>,
}
...
/// Generate sample inputs for fuzzer initialization
pub fn produce_samples() {
    ...
    fn write_sample(filename: &str, input: &TestInput) {
        ...
        // Try to create the file, but don't panic if it fails
        if let Ok(mut file) = File::create(filename) {
            let _ = file.write_all(&buffer[..usize::try_from(len).expect("Failed to convert length to usize")]);
        }
    }
