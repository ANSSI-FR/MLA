...
mod tests {
    ...
    fn _foo() {
        let (privkey, pubkey) = generate_mla_keypair();
        let mut outpriv = std::fs::File::create("out.priv").unwrap();
        privkey
            .get_signature_private_key()
            .serialize_signature_private_key(&mut outpriv)
            .unwrap();
        let mut outpub = std::fs::File::create("out.pub").unwrap();
        pubkey
            .get_signature_verification_public_key()
            .serialize_signature_verification_public_key(&mut outpub)
            .unwrap();
    }
}
