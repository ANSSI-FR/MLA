...
* Streamable archive creation:
  * An archive can be build even over a data-diode
  * A file can be added through chunks of data, without knowing the final size at first place
  * Files chunks can be interleaved (one can add the beginning of a file, starts a second one, and then continue adding first's file parts)
* Archive's files are seekable, even if compressed or encrypted. A file can be accessed in the middle of the archive without reading from the beginning
* If truncated, archive can be repaired. Files which were still in the archive, and the beginning of the ones for which the end is missing, will be recovered
...
// Starts a second file and add content
...
* a `Writer`, implementing the `Write` trait. It is responsible of emitting bytes while creating a new archive
* a `Reader`, implementing both `Read` and `Seek` traits. It is responsible of reading bytes while reading an archive
* a `FailSafeReader`, implementing only the `Read` trait. It is responsible of reading bytes while repairing an archive

Layers are made with the *repairable* aspect in mind. Reading them must never need information from the footer, but a footer can be used to optimize the reading. For example, accessing a file inside the archive can be optimized using the footer to seek to the file beginning, but it is still possible to get information by reading the whole archive until the file is found.
...
track of how many bytes have been written in the sub-layers.
...
This key `k` will further be used by for the symmetric encryption of the archive.
...
The layer data is then made of several encrypted block, each with a constant size except for the last one. Each block is encrypted with an IV including the base nonce and a counter. This construction is close to the [STREAM](https://github.com/miscreant/meta/wiki/STREAM) one, except for the `last_block` bit. The choice has been made to not use it, because:
...
Thus, to seek-and-read at a given position, the layer decrypts the block containing this position, and verify the tag before returning the decrypted data. 
...
* An already security audited Rust library exists for Curve 25519
...
stop when 4MB of cleartext has been obtained. Then it is reset, and starts
...
The layer footer contains for each file its size, its ending block offset and an index of its block locations. Block locations index enables direct access. The ending block offset enables fast hash retrieval and the file size eases the conversion to formats needing the size of the file before the data, such as Tar.
...
* data, which are a stream of byte

A few metadata are also computed, such as:
* the file size
* the SHA256 hash of the content

No additional metadata (permissions, ownership, etc.) are present, and would probably not be added unless very strong arguments are given. The goal is to keep the file format simple enough, and to leave the complexity to the code using it. Things such as permissions, ownership, etc. are hard to guarantee over several OS and filesystems; and leads to complexity, for example in tar. For the same reasons, `/` or `\` do not have any signification in filename; it is up to the user to choose how to handle them (are there namespaces? directories in Windows style? etc.).

If one still wants to have associated metadata for its own use case, the recommended way is to bring along an additional file in the archive containing the needed metadata.

Additionally, the file format is meant to change lightly in the future, to keep an easier backward compatibility, or, at least, version conversion, and simple support.
...
* `linear_extract`: Extract linearly an Archive. Faster way to extract a whole archive, by reducing the amount of costly `seek` operations
...
But to the best knowledge of the author, none of them support the aforementioned
features (but, of course, are better suitable for others purposes).

For instance (from the understanding of the author):

* `tar` format needs to known the size of files before adding them, and is not
  seekable
...
* Backup formats are generally written to avoid things such as duplication,
  and then needs to keep bigger structures in memory or are not streamable

Tweaking these formats would likely have resulted in similar properties. The
choice has been made to keep a better control on what the format is capable of,
and to (try to) KISS.
...
* File addition, with different size and different layers configuration
* File addition, varying the compression quality
* File reading, with different size and different layers configuration
* Random file read, with different size and different layers configuration
* Linear archive extraction, with different size and different layers configuration

On a "Intel(R) Core(TM) i5-7200U CPU @ 2.50GHz":
```sh
$ cd mla_archive/
$ cargo bench
...
...
* Create archives with interleaved files, and different layers enabled
* Read them to check their content
* Repair the archive without truncation, and verify it
* Alter the archive raw data, and ensure reading it does not panic (but only fail)
* Repair the altered archive, and ensure the repairing doesn't fail (only reports detected errors)
