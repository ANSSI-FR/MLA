...
with mla.MLAWrite("example.mla", config) as archive:
...
with mla.MLARead("example.mla", config) as archive:
...
- `mla.MLARead(path, config)` — Open an archive for reading
- `mla.MLAWrite(path, config)` — Open an archive for writing
