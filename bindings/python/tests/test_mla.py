...
import mla
from mla import MLARead, MLAWrite, MLAError, EntryName
...
def test_bad_mode():
    "Ensure MLAWrite/MLARead with wrong config type raises error"
    target_file = "/tmp/must_not_exists"
    with pytest.raises(TypeError):
        MLAWrite(target_file, "NOT_A_CONFIG")
    with pytest.raises(TypeError):
        MLARead(target_file, "NOT_A_CONFIG")
...
def test_repr():
    ...
    archive = MLAWrite(path, mla.WriterConfig.without_encryption_without_signature())
    assert repr(archive) == f"<MLAWrite path='{path}'>"
    archive.finalize()
    archive = MLARead(
        path,
    ...
    assert repr(archive) == f"<MLARead path='{path}'>"
...
def test_context_write():
    ...
    with MLAWrite(path, mla.WriterConfig.without_encryption_without_signature()) as m:
        for name, data in FILES.items():
            m[name] = data
...
def test_mlafile_bad_config():
    "Try to create a MLAWrite/MLARead with the wrong config parameter"
    with pytest.raises(TypeError):
        MLAWrite(tempfile.mkstemp(suffix=".mla")[1], "NOT A CONFIG")
    with pytest.raises(TypeError):
        MLAWrite(
            tempfile.mkstemp(suffix=".mla")[1],
            mla.ReaderConfig.without_encryption(
