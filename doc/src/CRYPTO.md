...
- This secret is unique per archive, as it is generated on archive creation. Even "converting" or "recovering" an archive in `mlar` CLI will force a newly fresh secret. It is a new secret as there is no edit feature implemented, even if it is doable. Hence, a new random symmetric key is used to encrypt its content while "converting" or "recovering" an archive.
